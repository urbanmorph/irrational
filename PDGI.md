...
The field guide, the sources and the audit *procedure* are plain content: the [/biases](https://irrational.pages.dev/biases) cards use native HTML `<details>` and read without JavaScript, and the underlying method (a System 1 / System 2 audit, the premortem, the outside view) is a pen-and-paper practice anyone can run from the printed questions. Only the live demo and the forge need JavaScript.
Direction: ship a printable one-page audit checklist and a downloadable catalogue, so the method is usable entirely offline.

### Grassroots and reaching the divide-affected — ⛳
Today's catalogue is English, and the audience skews toward builders and decision-makers. Vernacular access is not built.
Direction: the bring-your-own-model design already makes multilingual answers reachable — the scaffold can instruct the user's AI to audit and reply in any language. The next step is to translate the field guide itself and make the language instruction first-class in the MCP.
